...
def run(cfg, network, imagedir, calib, stride=1, viz=False):
    ...
    reader = Process(target=image_stream,
                     args=(queue, imagedir, calib, stride, 0))
    reader.start()
...
if __name__ == '__main__':
    ...
    for scene in euroc_scenes:
        ...
        for i in range(args.trials):
            traj_est, timestamps = run(cfg, args.network, imagedir,
                                       "calib/euroc.txt", args.stride,
                                       args.viz)
            images_list = sorted(glob.glob(os.path.join(
                imagedir, "*.png")))[::args.stride]
            ...
            # print("traj_est ", traj_est.get_infos())
            # print("traj_ref ", traj_ref.get_infos())
            # print(
            #     "diff ",
